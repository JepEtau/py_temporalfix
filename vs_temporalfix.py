...
def AverageColorFix(clip, ref, radius=4, passes=4):
    ...
    blurred_reference = core.std.BoxBlur(
        ref,
        hradius=radius, hpasses=passes,
        vradius=radius, vpasses=passes
    )
    blurred_clip = core.std.BoxBlur(
        clip, hradius=radius, hpasses=passes, vradius=radius, vpasses=passes
    )
...
def AverageColorFixFast(clip, ref, downscale_factor=8):
    ...
    downscaled_reference = core.resize.Bilinear(
        ref,
        width=clip.width/downscale_factor,
        height=clip.height/downscale_factor
    )
...
def TweakDarks(src, s0=2., c=0.0625, chroma=True):
    ...
    x = f"x {i} /" if bd != 8 else "x"
    expr = 'x' if isFLOAT else f"{x} {i} *"
    k = (s0 - 1) * c
    t = f"{x} 255 / 0 max 1 min"
    c1 = 1 + c
    c2 = c1 * c
    e = f"{k} {c1} {c2} {t} {c} + / - * {t} 1 {k} - * + {256 * i} *"
    return core.std.Expr(
        [src],
        [e] if src.format.num_planes == 1 else [e, expr if chroma else '']
    )
...
def vs_temporalfix(clip, strength=400, tr=6, exclude=None, debug=False):
    ...
    mv_super = core.mvsf.Super if mvtr > 6 else core.mv.Super
    mv_analyze = core.mv.Analyse
    mv_recalculate = core.mv.Recalculate
    mv_degrain1 = core.mv.Degrain1
    mv_degrain2 = core.mv.Degrain2
    mv_degrain3 = core.mv.Degrain3
    mv_degrain4 = core.mv.Degrain4
    mv_degrain5 = core.mv.Degrain5
    mv_degrain6 = core.mv.Degrain6
    bd = 16
    ...
    clip = core.std.AddBorders(
        clip, left=border_add, right=border_add, top=border_add, bottom=border_add
    )
    if original_format != vs.YUV444P16:
        clip = core.resize.Point(clip, format=vs.YUV444P16)

    clip = core.fb.FillBorders(
        clip, left=border_add, right=border_add, top=border_add, bottom=border_add, mode="fillmargins", interlaced=0
    )
    pre_stabilize = clip
    ...
    motionmask_super = core.mv.Super(
        motionmask_pref,
        pel=2,
        sharp=1,
        rfilter=4,
    ...
    motionmask_vectors = core.mv.Analyse(
        motionmask_super,
        isb=False,
        delta=1,
        blksize=128,
    ...
    motionmask_window = core.mv.Compensate(
        motionmask_pref,
        motionmask_super,
        motionmask_vectors,
        thsad=200000,
    ...
    motionmask_window = core.motionmask.MotionMask(
        motionmask_window,
        th1=[45],
        th2=[40],
        tht=33,
    ...
    motionmask = core.std.Expr(
        [
            motionmask,
            motionmask_shift_minus_one,
            motionmask_shift_minus_two,
            motionmask_shift_minus_three,
            motionmask_shift_plus_one,
            motionmask_shift_plus_two
        ],
        expr=['x y + z 0.75 * + a 0.5 * + b 0.75 * + c 0.5 * +']
    )
    ...
    if pel > 1:
        ...
        pre_stabilize_resized = core.resize.Bilinear(
            pre_stabilize, format=vs.YUV444P8, width=clip.width * 2, height=clip.height * 2
        )
    ...
    #superclip
    if pel > 1:
        super_search = mv_super(prefilter, chroma=chroma, rfilter=4, pelclip=pelclip, pel=pel)
    else:
        super_search = mv_super(prefilter, chroma=chroma, rfilter=4, sharp=1, pel=pel)
    super_render = mv_super(clip, chroma=chroma, rfilter=1, sharp=subpixel, levels=1, pel=pel)
    ...
    if mvtr < 7:
        if mvtr > 5:
            bv6 = mv_analyze(super_search, isb=True,  delta=6, **analyse_args)
            fv6 = mv_analyze(super_search, isb=False, delta=6, **analyse_args)
        if mvtr > 4:
            bv5 = mv_analyze(super_search, isb=True,  delta=5, **analyse_args)
            fv5 = mv_analyze(super_search, isb=False, delta=5, **analyse_args)
        if mvtr > 3:
            bv4 = mv_analyze(super_search, isb=True,  delta=4, **analyse_args)
            fv4 = mv_analyze(super_search, isb=False, delta=4, **analyse_args)
        if mvtr > 2:
            bv3 = mv_analyze(super_search, isb=True,  delta=3, **analyse_args)
            fv3 = mv_analyze(super_search, isb=False, delta=3, **analyse_args)
        bv1 = mv_analyze(super_search, isb=True,  delta=1, **analyse_args)
        fv1 = mv_analyze(super_search, isb=False, delta=1, **analyse_args)
        if mvtr > 1:
            bv2 = mv_analyze(super_search, isb=True,  delta=2, **analyse_args)
            fv2 = mv_analyze(super_search, isb=False, delta=2, **analyse_args)
    else:
        if mvsflegacy:
    ...
    if mvtr < 7:
        ...
        if mvtr == 6:
            clip = mv_degrain6(clip, super_render, bv1, fv1, bv2, fv2, bv3, fv3, bv4, fv4, bv5, fv5, bv6, fv6, **degrain_args)
        elif mvtr == 5:
            clip = mv_degrain5(clip, super_render, bv1, fv1, bv2, fv2, bv3, fv3, bv4, fv4, bv5, fv5, **degrain_args)
        elif mvtr == 4:
            clip = mv_degrain4(clip, super_render, bv1, fv1, bv2, fv2, bv3, fv3, bv4, fv4, **degrain_args)
        elif mvtr == 3:
            clip = mv_degrain3(clip, super_render, bv1, fv1, bv2, fv2, bv3, fv3, **degrain_args)
        elif mvtr == 2:
            clip = mv_degrain2(clip, super_render, bv1, fv1, bv2, fv2, **degrain_args)
        else:
            clip = mv_degrain1(clip, super_render, bv1, fv1, **degrain_args)
    else:
        degrain_args = dict(thsad=[thSAD, thSADC, thSADC], plane=plane, limit=[limit, limitc, limitc], thscd1=thSCD1, thscd2=thSCD2)
        if mvsflegacy:
    ...
    edgemasks_diff = core.std.Levels(edgemasks_diff, max_in=32768-1, max_out=65536-1)
...
def Recalculate(super, vectors, thsad=200.0, smooth=1, blksize=8, blksizev=None, search=4, searchparam=2, lambda_=None, chroma=True, truemotion=True, pnew=None, overlap=0, overlapv=None, divide=0, meander=True, fields=False, tff=None, dct=0, tr=3):
    ...
    def refine(delta):
        ...
        return core.mvsf.Recalculate(super, analyzed, thsad=thsad, smooth=smooth, blksize=blksize, blksizev=blksizev, search=search, searchparam=searchparam, lambda_=lambda_, chroma=chroma, truemotion=truemotion, pnew=pnew, overlap=overlap, overlapv=overlapv, divide=divide, meander=meander, fields=fields, tff=tff, dct=dct)

    return core.std.Interleave([refine(i) for i in range(0, 2*tr)])
...
def ContraSharpening(clip, src, radius=None, rep=24, planes=[0, 1, 2]):
    ...
    expr = f"x {mid} - abs y {mid} - abs < x y ?" # abs(diff) after limiting may not be bigger than before
